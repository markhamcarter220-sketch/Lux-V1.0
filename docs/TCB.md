...
| Assumption | Source evidence | Status |
|------------|-----------------|--------|
| Lean 4 theorem `concreteDeductSpec` proves the `deduct` function satisfies `AbstractLedger.Spec` — balance monotonically decreases by exactly the requested amount, all other nodes unchanged. | `lean/LuxRefinement.lean:17-26` | [UNVERIFIED ASSUMPTION] — statement contains no `sorry` token (source inspection); `lake build` has NOT been run in this session, so the proof is written but not mechanically checked |
| Lean 4 theorem `delegate_non_amplification` proves that delegation never produces a token with rights strictly exceeding the delegator's rights. | `lean/LuxRefinement.lean:28-36` | [UNVERIFIED ASSUMPTION] — statement contains no `sorry` token (source inspection); `lake build` has NOT been run in this session, so the proof is written but not mechanically checked |
...
| Mechanical verification of the system-level I1–I4 obligation layer | The Lean 4 proof suite (four core modules plus `lean/Refinement.lean`) is written but has NOT been mechanically verified — no `lake build` has been witnessed in this session. Source inspection (grep, not a build) finds no `sorry` token in the four core modules and exactly 2 literal `sorry` tokens in `lean/Refinement.lean` (both I3 obligations: `accountableResources_soleDeductionPath`, `accountableResources_ceilingBound`). The I4 theorems have proof terms but with unproven hypotheses (see `docs/REFINEMENT_GAPS.md §I4`). A separate file `lean/IpcReservation.lean` carries 5 additional `sorry` tokens (IPC protocol obligations, not part of the I1–I4 invariant set). See `docs/REFINEMENT_GAPS.md`. |
| Externally audited security | The codebase is internally reviewed only. Four specific human-review items remain outstanding per `CLAUDE.md`, including the `lake build` mechanical-verification item, which has NOT been completed. |
