...
fn attack_5_6_quota_exhaustion_does_not_cascade_to_peer_nodes() {
    let mut ledger = Ledger::new();
    ledger
        .seed(nz(1), Quota::new(10))
        .expect("test node count within MAX_NODES");
    ledger
        .seed(nz(2), Quota::new(10))
        .expect("test node count within MAX_NODES");
...
fn attack_5_8_audit_log_at_capacity_no_overwrite_chain_intact() {
    ...
    assert_eq!(
        ...
        u32::try_from(MAX_AUDIT_EVENTS - 1).expect("constant fits in u32")
    );
}
