...
proptest! {
    ...
    fn undeclared_edge_always_denied(
        ...
        if n_s1 != n_d1 {
            prop_assert!(
                graph.traverse(n_d1, n_s1, &mut AuditLog::new()).is_err(),
        ...
        if n_s2 != n_s1 || n_d2 != n_d1 {
            prop_assert!(
                graph.traverse(n_s2, n_d2, &mut AuditLog::new()).is_err(),
    ...
    fn declared_active_edge_always_permitted(
        ...
        prop_assert!(
            graph.traverse(ns, nd, &mut AuditLog::new()).is_ok(),
    ...
    fn inactive_node_blocks_edge_declaration(
        ...
        let graph = booting.seal();
        prop_assert!(
            graph.traverse(ns, nd, &mut AuditLog::new()).is_err(),
