...
fn revoke_live_token_denies_immediately() {
    ...
    let c = cap(100, gen);
    assert_eq!(
        policy.check(&c, CapabilitySet::SCHEDULE),
...
fn non_revoked_token_is_permitted() {
    ...
    let c = cap(200, gen);
    assert!(policy.check(&c, CapabilitySet::SCHEDULE).is_ok());
}
...
fn revocation_checked_before_nonce_consumption() {
    ...
    let r1 = policy.check(&c, CapabilitySet::SCHEDULE);
    ...
    let c2 = cap(77, gen1);
    assert!(policy.check(&c2, CapabilitySet::SCHEDULE).is_ok());
}
...
fn revocation_does_not_affect_different_nonces() {
    ...
    for n in 4..=6u64 {
        let c = cap(n, gen);
        assert!(policy.check(&c, CapabilitySet::SCHEDULE).is_ok(), "nonce {n} should pass");
    }
}
...
fn multiple_revocations_all_deny() {
    ...
    for n in 0..20u64 {
        let c = cap(n, gen);
        assert!(
            policy.check(&c, CapabilitySet::SCHEDULE).is_err(),
